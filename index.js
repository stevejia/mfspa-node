...
function log(text) {
  ...
  console.info(`%c[${time.toLocaleTimeString()}]`, "color: red; font-size: 30px;", text);
}
...
const sendMessageToOthers = (currentID, message) => {
  connectionArray.forEach((connection, index) => {
    if (connection.clientID !== currentID) {
...
wsServer.on("request", function (request) {
  ...
  var msg = {
    ...
    currentUsers: connectionArray.length,
  };
  ...
  connection.on("message", function (message) {
    if (message.type === "utf8") {
      ...
      switch (msg.type) {
        ...
        case "candidate":
          //candidate
          const { id: cId, candidate } = msg;
          sendMessageToOthers(cId, { type: "candidate", candidate });
          break;
        //发送rtc offer
        case "giveoffer":
          const { id, sdp } = msg;
          sendMessageToOthers(id, { type: "giveoffer", sdp });
          break;
        //回送rtc offer
        case "backoffer":
          const { id: clientId, sdp: backSdp } = msg;
          sendMessageToOthers(clientId, { type: "backoffer", sdp: backSdp });
          break;
      }
...
let timer = null;
  const heartBeat = () => {
    timer = setInterval(() => {
      sendMessageToAll({ type: "heartbeat", msg: "heartbeat" });
    }, 5000);
  };
